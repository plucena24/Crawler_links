...
class Spider():
    ...
    def venom(egg):
        ...
        a_current = urlparse(egg)
        if not 'eniyihekim.com' in a_current.netloc:
            print("DEBUG")
            print(egg)
            return title, description, web
        ...
        for link in dom.xpath('//a/@href'):
            ...
            if 'javascript' not in a.netloc:
                ...
                if not 'eniyihekim.com' in a.netloc:
                    continue
    ...
    def insert_urls_db(urls_array):
        ...
        try:
            ...
            for data in urls_array:
                #print(item)
                cur.execute("INSERT OR IGNORE INTO Urls VALUES ('%s', NULL)" % data)
    ...
    def select_urls_db():
        ...
        try:
            ...
            cur.execute("SELECT Url FROM Urls LIMIT 100")
            rows = cur.fetchall()
            for row in rows:
                #print row
                result.append(row)
    ...
    def update_urls_db(upd):
        ...
        try:
            ...
            con = lite.connect('test.db')
            cur = con.cursor()
            cur.execute("UPDATE Urls SET State = 1 WHERE Url = '%s'" % upd)
